...
| FR-EVT-002 | Dedupe across restarts | yes | `focus-storage/src/sqlite/` | SQLite event store with `DedupeKey` unique index (migration v1). |
| FR-EVT-003 | Cursor persistence per (connector_account, entity_type) | yes | `focus-sync/src/lib.rs`, `focus-storage/src/sqlite/cursor_store.rs` | `SqliteCursorStore` + `SyncOrchestrator::with_cursor_store` hydrates on `register` and persists every successful sync; restart-survival regression test in `focus-sync`. |
...
| FR-RULE-004 | RuleEvaluation with explanation | yes | `focus-rules/src/lib.rs` | `RuleEngine::evaluate_with_trace` / `evaluate_schedule_tick_with_trace` / `evaluate_state_change_with_trace` construct full records with `explanation` + `state_snapshot_ref`. |
...
| FR-STATE-003 | Mutations append-only | yes | `focus-rewards`, `focus-penalties`, `focus-policy` | Every aggregate `apply` takes `&dyn AuditSink` and writes `wallet.*`/`penalty.*`/`policy.*` records. `SqliteAuditStore` persists the chain (migration v3). |
...
| FR-ENF-004 | Policy activation audited | yes | `focus-policy/src/lib.rs` | `PolicyBuilder::from_rule_decisions` writes a `policy.built` audit line on every construction, payload carries `policy_id` + `active` + `profile_states` + `decision_ids`. `SqliteAuditStore` persists. |
...
| FR-DATA-002 | All mutations append AuditRecord | yes | `focus-audit`, `focus-rewards`, `focus-penalties`, `focus-policy`, `focus-rituals` | `AuditSink` threaded through every aggregate `apply` (rewards/penalties/policy) and `RitualsEngine::capture_intention`. `CapturingAuditSink` tests verify payload shape per crate. |
...
| FR-UX-002 | Native OAuth flow (SFSafariVC / Custom Tabs) | yes | `apps/ios/.../Settings/CanvasAuthView.swift` | `ASWebAuthenticationSession` drives Canvas OAuth; code handed to `FocalPointCore.connector().connectCanvas(...)` which exchanges via `CanvasOAuth2` and persists via `KeychainStore`. |
...
| FR-CAL-001 | CalendarPort trait + EventKit + GCal | yes | `focus-calendar/src/lib.rs`, `focus-ffi/src/lib.rs`, `apps/ios/.../EventKitCalendarHost.swift`, `connector-gcal/` | `CalendarHost` UniFFI callback interface + `HostBackedCalendarPort`; Swift `EventKitCalendarHost` reads the iOS device calendar via `EKEventStore`. `NSCalendarsFullAccessUsageDescription` in Info.plist. `connector-gcal` covers Google Calendar via OAuth2 + keychain. Original 4 InMemory tests still green. |
