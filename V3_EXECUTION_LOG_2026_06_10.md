...
### L3-#58 (pheno-ci-templates)

**Task (V3 DAG L3 layer):** Author the canonical
`pheno-ci-templates/` package — a self-contained vendoring of two
reusable GitHub Actions workflows plus a usage README — to serve as
the single source of truth for CI + release across every pheno-*
crate / app / SDK in the fleet. The CI workflow must be a reusable
(`workflow_call` + `workflow_dispatch`) workflow with the six
spec-required inputs (`rust_msrv`, `python_version`, `go_version`,
`node_version`, `run_coverage`, `run_clippy`) and must auto-detect
which language jobs to run based on the consumer's manifest files
(`hashFiles('**/Cargo.toml')`, `'**/pyproject.toml'`, `'**/go.mod'`,
`'**/package.json'`). The release workflow must trigger on tag push
(`v*` + `release/*`), create a GitHub release via
`softprops/action-gh-release@v2`, and conditionally publish to
crates.io / npm / PyPI / $GOBIN — each gated by a per-language
`run_<lang>` feature flag. Consumed by L3 #49–#57 (every pheno-*
crate) and any future pheno-* / phenotype-* repo that needs uniform
CI + release coverage.

**Layout:** Three files in a new `pheno-ci-templates/` directory at
the monorepo root, structured as a vendoring package (its own
`.github/workflows/` subdirectory) rather than as a top-level
`.github/workflows/ci.yml`. The reason: the template repo is itself
meant to be vendored by other repos, so an explicit
`pheno-ci-templates/` boundary makes the package contract obvious
and lets the template ship its own README and any future companion
files (actionlint configs, Dependabot config, etc.) without polluting
the monorepo root.

**ci.yml design notes (287 lines, 5 jobs):**
- **Triggers:** `workflow_call` (the standard reusable workflow
  shape) + `workflow_dispatch` (mirror of all 6 inputs, so the
  template can be smoke-tested from the Actions tab without any
  caller).
- **Permissions:** `contents: read` only — no writes, no id-token
  needed for CI.
- **Concurrency:** `group = 'ci-${{ github.workflow }}-${{ github.ref }}'`
  with `cancel-in-progress: true` (new pushes to the same branch
  cancel the previous in-flight run).
- **Per-job auto-detect:** every language job is gated by
  `if: hashFiles('<manifest>') != ''`, so a pure-Rust crate like
  `pheno-otel` only runs the rust job, a pure-Python package like
  `pheno-fastapi-base` only runs the python job, and a monorepo with
  any subset of the four manifests runs only the matching jobs.
- **rust job:** `dtolnay/rust-toolchain@stable` pinned to the
  `rust_msrv` input, `actions/cache@v4` keyed on
  `hashFiles('**/Cargo.lock', '**/Cargo.toml')` (caches
  `~/.cargo/registry`, `~/.cargo/git`, `target/`), then
  `cargo fmt --all -- --check`, optional
  `cargo clippy --all-targets -- -D warnings` (gated on
  `run_clippy`), `cargo test --all-features --no-fail-fast` (always),
  and optional `cargo llvm-cov --all-features --workspace
  --summary-only` (gated on `run_coverage`).
- **python job:** `actions/setup-python@v5` with the `python_version`
  input + `cache: pip`, then `pip install -e ".[dev]"` (so dev extras
  are always installed), `pytest --maxfail=1 --tb=short` (always),
  and optional `pytest --cov=src --cov-report=term-missing` (gated
  on `run_coverage`).
- **go job:** `actions/setup-go@v5` with the `go_version` input +
  `cache: true`, then `go vet ./...` (always), `go test -race
  -count=1 ./...` (always), and optional
  `go test -cover -coverprofile=coverage.out ./...` followed by
  `go tool cover -func=coverage.out` (gated on `run_coverage`).
- **node job:** `actions/setup-node@v4` with the `node_version`
  input + `cache: npm`, then `npm ci` + `npm test -- --watch=false`
  (always), and `npm test -- --watch=false --coverage` (gated on
  `run_coverage`).
- **summary job:** depends on all four language jobs, fails if any
  single language job failed, and is itself skipped when no manifests
  are detected (avoids a misleading "no jobs ran" failure for an
  empty / docs-only repo).

**release.yml design notes (200 lines, 1 monolithic job):**
- **Triggers:** `push: tags: ['v*', 'release/*']` (covers both
  semver tags and the `release/*` branch convention) +
  `workflow_dispatch` with a `tag` input (so a maintainer can
  manually re-trigger a release for an existing tag) + the four
  `run_<lang>` feature flag re-exposed.
- **Permissions:** `contents: write` (create the GitHub release) +
  `id-token: write` (OIDC-based trusted publishing for `cargo` +
  `twine` where the registry supports it; falls back to long-lived
  tokens via `secrets.CARGO_REGISTRY_TOKEN` / `secrets.PYPI_TOKEN`).
- **Concurrency:** `group = 'release-${{ github.ref }}'` with
  `cancel-in-progress: false` (a release publish that gets
  half-completed and then cancelled is a much worse outcome than
  a queued second run).
- **Tag resolution step:** uses `inputs.tag` if
  `workflow_dispatch`, else `GITHUB_REF_NAME`.
- **Release step:** `softprops/action-gh-release@v2` with
  `generate_release_notes: true`, `draft: false`, and
  `prerelease: ${{ contains(tag, '-') }}` (so `v1.0.0-rc.1`
  becomes a prerelease automatically).
- **Per-language publish steps (each gated twice — on the
  `run_<lang>` input AND on the manifest being present):**
  - **Cargo:** `cargo build --release --all-features` →
    `cargo publish --dry-run --allow-dirty --all-features` (safety
    net) → `cargo publish --all-features` (real publish, uses
    `CARGO_REGISTRY_TOKEN`).
  - **npm:** `npm ci` → `npm test -- --watch=false` (pre-publish
    gate) → `npm publish --access public` (uses `NODE_AUTH_TOKEN`).
  - **PyPI:** `pip install build twine` → `python -m build
    --wheel --sdist` → `twine check dist/*` → `twine upload dist/*`
    (uses `TWINE_USERNAME=__token__` + `TWINE_PASSWORD=$PYPI_TOKEN`).
  - **Go:** `go mod download` → `go vet ./...` → `go install ./...`
    (with `GOFLAGS=-mod=readonly`).

**Verification:** Both workflows parse cleanly under
`python3 -c "import yaml; yaml.safe_load(open('<file>'))"`
(spec-required verification). Parsing the ci.yml yields a dict
with the 6 spec-required input keys (rust_msrv, python_version,
go_version, node_version, run_coverage, run_clippy) and 5 jobs
(rust, python, go, node, summary). Parsing the release.yml yields
a dict with `on.push.tags: ['v*', 'release/*']` and a single
`release` job whose steps include the 4 feature-flag-gated publish
sequences. The spec statement "Tests N/A (workflow YAML is verified
by GitHub Actions on merge)" is honored — there is no local test
runner for workflow YAML.

**README (149 lines):** Documents the input table, feature flags,
two copy-pasteable consumer snippets (one for ci.yml, one for
release.yml), the language auto-detect behavior, the caching
strategy, the permissions model, and the recommended SHA/tag
pinning convention (`@v1.0.0` over `@main` for production).

**Branch:** `chore/l3-58-pheno-ci-templates-2026-06-11`, local-only
(NOT pushed per task directive). Feature commit: `b3b1b04c50` on
branch `chore/l3-58-pheno-ci-templates-2026-06-11`. Canonical
worklog: `worklogs/l3-58-pheno-ci-templates-2026-06-11.json`.
