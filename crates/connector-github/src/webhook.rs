...
use focus_connectors::{FocusError, Result, WebhookDelivery, WebhookHandler};
...
impl WebhookHandler for GitHubWebhookHandler {
    async fn handle(&self, delivery: &WebhookDelivery) -> Result<Vec<NormalizedEvent>> {
        ...
        let payload: Value = serde_json::from_slice(&delivery.body)
            .map_err(|e| FocusError::Schema(format!("invalid json: {}", e)))?;
        ...
        let gh_event = GitHubEvent {
            id: payload
                .get("id")
                .and_then(|v| v.as_u64())
                .ok_or_else(|| FocusError::Schema("missing 'id' field".to_string()))?
                .to_string(),
            event_type: payload
                .get("type")
                .and_then(|v| v.as_str())
                .ok_or_else(|| FocusError::Schema("missing 'type' field".to_string()))?
                .to_string(),
