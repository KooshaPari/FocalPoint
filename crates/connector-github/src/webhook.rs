...
impl WebhookHandler for GitHubWebhookHandler {
    async fn handle(&self, delivery: &WebhookDelivery) -> Result<Vec<NormalizedEvent>> {
        ...
        let payload: Value = serde_json::from_slice(&delivery.body)
            .map_err(|e| ConnectorError::invalid_input("connector", format!("invalid json: {}", e)))?;
        ...
        let gh_event = GitHubEvent {
            id: payload
                .get("id")
                .and_then(|v| v.as_u64())
                .ok_or_else(|| ConnectorError::invalid_input("connector", "missing 'id' field".to_string()))?
                .to_string(),
            event_type: payload
                .get("type")
                .and_then(|v| v.as_str())
                .ok_or_else(|| ConnectorError::invalid_input("connector", "missing 'type' field".to_string()))?
                .to_string(),
